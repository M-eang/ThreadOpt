...
双档位自动跟随前台应用切换：前台是 `game.conf` 中的游戏包名 → 自动切**游戏档**（帧率优先，线程全上性能核）；回到桌面/日常应用 → 自动切回**省电档**（后台收紧、前台宽松）。

- 省电档配置：`applist.conf`（默认 `-c` 指定）；游戏档配置：`game.conf`（`-g` 指定，不存在则档位切换禁用）
- 前台判定依据系统 `oom_score_adj`（前台活动 adj≈0，后台 ≥200）
- 手动兜底：配置目录建 `mode` 文件，内容 `auto` / `power` / `game` 强制锁定
- `game.conf` 的包名既是规则也是前台检测名单，模块内置常用游戏模板
