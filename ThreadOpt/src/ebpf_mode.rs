//! eBPF 事件驱动模式（仅 ebpf feature 启用时编译；关闭时回退纯 /proc 轮询）
#![cfg(feature = "ebpf")]
...
use threadopt::config::{AppConfig, CURRENT_CONFIG};
use threadopt::cpuset::CpuSet;
...
fn comm_str<'a>(comm: &'a [u8; 16]) -> &'a str {
...
pub fn ebpf_init() -> Option<EbpfState> {
    ...
    let ebpf_path = std::env::current_exe()
        .ok()
        .and_then(|p| p.parent().map(|p| p.join("ThreadOpt-ebpf")))?;
    ...
    let pkgs_len = threadopt::lock_ignore_poison(&CURRENT_CONFIG)
        .as_ref()
...
fn event_apply(
    ...
    cache.task_apply(tid, pid, &pkg, comm, has_thread_rules, cfg, |t, c, d| {
